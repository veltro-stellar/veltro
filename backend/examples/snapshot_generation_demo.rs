...
use veltro_backend::services::contract::{ContractConfig, ContractService};
use veltro_backend::services::snapshot::SnapshotService;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let database_url =
        std::env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite:veltro.db".to_string());
