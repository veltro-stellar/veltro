...
async fn test_replay_range() {
    use veltro_backend::replay::config::ReplayRange;
...
async fn test_replay_storage() {
    ...
    // Create metadata
    let metadata = veltro_backend::replay::ReplayMetadata {
        session_id: "test-session".to_string(),
        config: ReplayConfig::default(),
        status: veltro_backend::replay::ReplayStatus::Pending,
        started_at: Utc::now(),
        ended_at: None,
        checkpoint: None,
    };
