...
curl -X POST "$KIBANA_URL/api/saved_objects/index-pattern/veltro-logs" \
  -H 'kbn-xsrf: true' \
...
curl -X POST "$KIBANA_URL/api/saved_objects/index-pattern/veltro-errors" \
  -H 'kbn-xsrf: true' \
  -H 'Content-Type: application/json' \
  -d '{
    "attributes": {
...
curl -X PUT "$ELASTICSEARCH_URL/_ilm/policy/veltro-ilm-policy" \
  -H 'Content-Type: application/json' \
  -d '{
    "policy": {
      "phases": {
...
curl -X PUT "$ELASTICSEARCH_URL/_index_template/veltro-template" \
  -H 'Content-Type: application/json' \
  -d '{
    "index_patterns": ["veltro-*"],
    "template": {
      "settings": {
        "number_of_shards": 1,
        "number_of_replicas": 0,
        "index.lifecycle.name": "veltro-ilm-policy",
        "index.lifecycle.rollover_alias": "veltro",
        "refresh_interval": "5s"
      },
      "mappings": {
        "properties": {
...
echo "  - Specific service: service:\"veltro\""
