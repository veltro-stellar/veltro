...
echo '{"@timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%S.%3NZ)'","log_level":"info","message":"ELK test log","service":"veltro","target":"test"}' | nc localhost 5000
sleep 2
...
sleep 3
if curl -s "http://localhost:9200/veltro-*/_search?q=message:ELK%20test%20log" | grep -q '"hits":{"total":{"value":[1-9]'; then
...
curl -s "http://localhost:9200/_cat/indices/veltro-*?v&h=index,docs.count,store.size"
