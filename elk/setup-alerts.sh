...
curl -X POST "$KIBANA_URL/api/alerting/rule" \
  ...
  -d '{
    "name": "High Error Rate - Veltro",
    "tags": ["veltro", "errors", "critical"],
...
curl -X POST "$KIBANA_URL/api/alerting/rule" \
  ...
  -H 'Content-Type: application/json' \
  -d '{
    "name": "Slow API Responses - Veltro",
    "tags": ["veltro", "performance", "warning"],
    "rule_type_id": ".es-query",
    "consumer": "alerts",
    "schedule": {
      "interval": "5m"
...
curl -X POST "$KIBANA_URL/api/alerting/rule" \
  ...
  -H 'Content-Type: application/json' \
  -d '{
    "name": "RPC Call Failures - Veltro",
    "tags": ["veltro", "rpc", "critical"],
    "rule_type_id": ".es-query",
    "consumer": "alerts",
    "schedule": {
      "interval": "5m"
...
curl -X POST "$KIBANA_URL/api/alerting/rule" \
  ...
  -H 'Content-Type: application/json' \
  -d '{
    "name": "Database Errors - Veltro",
    "tags": ["veltro", "database", "critical"],
    "rule_type_id": ".es-query",
    "consumer": "alerts",
    "schedule": {
      "interval": "5m"
