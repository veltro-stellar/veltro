...
export function CorridorComparisonTable({ corridors, onExport }: CorridorComparisonTableProps) {
  ...
  const getCellColor = (value: number, metric: MetricConfig) => {
    ...
    return 'bg-card dark:bg-slate-800';
  };
  ...
  return (
    <div className="bg-card dark:bg-slate-800 rounded-lg shadow-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
            ...
            {sortedCorridors.map((corridor, idx) => (
              <tr
                ...
                className={`hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors ${
                  idx % 2 === 0 ? 'bg-card dark:bg-slate-800' : 'bg-muted/20/50 dark:bg-slate-800/50'
                }`}
              >
                <th
                  scope="row"
