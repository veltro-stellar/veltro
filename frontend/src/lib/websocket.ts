/**
 * WebSocket client for real-time updates from the Veltro backend
 */
...
let wsInstance: VeltroWebSocket | null = null;
...
export function getWebSocketInstance(
  ...
  if (!wsInstance) {
    wsInstance = new VeltroWebSocket(config);
  }
  return wsInstance;
}
