...
echo -e "${BLUE}=== Veltro Kubernetes Deployment ===${NC}"
echo ""
...
if [ "$DRY_RUN" == "true" ]; then
    ...
    if [[ "$response" =~ ^([yY][eE][sS]|[yY])$ ]]; then
        ...
        kubectl rollout status deployment/veltro-backend -n veltro --timeout=5m
        kubectl rollout status deployment/veltro-frontend -n veltro --timeout=5m
        ...
        kubectl get pods -n veltro
        
        echo ""
        echo "=== Services ==="
        kubectl get svc -n veltro
        
        echo ""
        echo "=== Ingress ==="
        kubectl get ingress -n veltro
    else
